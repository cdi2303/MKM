...
`;
        ...
        .catch(() => alert("SEO 분석 도중 오류가 발생했습니다."));
});
...
document.getElementById('upgradeContentBtn')?.addEventListener('click', () => {
    ...
    fetch(window.ROUTES.generateUpgrade, {
        ...
        .then(data => {
            ...
            if (!data || !data.html) {
                Swal.fire("오류", "콘텐츠 개선 결과가 올바르지 않습니다.", "error");
                return;
            }
...
<div class="text-left">
    ...
    <div class="p-3 border rounded bg-gray-50" style="max-height: 300px; overflow-y: auto;">
        ${data.diff ?? '<em>비교 결과 없음</em>'}
    </div>
</div>
`,
        ...
        .catch(() => Swal.fire("오류", "SEO 자동 개선 처리 실패", "error"));
});
...
window.generateABTitles = function () {
    ...
    fetch(window.POST_DATA.generateTitleUrl, {
        ...
        .then(r => r.json())
        .then(d => {
            Swal.fire("완료", "AB 테스트용 제목 5개 생성 완료!", "success");
            console.log(d.titles);
        })
        .catch(() => Swal.fire("오류", "AB 제목 생성 실패", "error"));
};
